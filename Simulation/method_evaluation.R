...
#############################################################
# Evluation the Performance of All the Methods              #
...
w_true <- unlist(read.table("./True_para/w_syn.txt"))
...
# identified intrinsic genes by BUSseq
...
# Silhouette coefficients of the 15 CCs inferred by Seurat
sil_Seurat <- silhouette_cal(extract.cca, w_Seurat)

# Silhouette coefficients of the 10-component W inferred by ZINB-WaVE
...
data_raw <- NULL
for(b in 1:B){
  data_raw <- cbind(data_raw, SimulatedCounts[[b]])
}

set.seed(123)
all.dists.unc <- as.matrix(dist(t(log1p(data_raw))))
...
# legend infomation 
batch_name<-paste0("Batch ",1:B)
celltype_name<-paste0("Cell type ",1:K)
...
#####
# Stop at here and save.image
#####
##############################################
# T-SNE plot colored by estimated cell types #
##############################################
# Interpolating the set of cell type colors to represent more cell types 
color_by_celltype_ramp<-colorRampPalette(color_by_celltype)(13)
# Notice that color_by_celltype is the same as color_by_celltype_ramp[c(1,4,7,10,13)]
...
w_BUSseq_shift[which(w_BUSseq=="5")]<-2#2 in w
w_BUSseq_shift[which(w_BUSseq=="2")]<-3#3 in w
w_BUSseq_shift[which(w_BUSseq=="4")]<-4#4 in w
w_BUSseq_shift[which(w_BUSseq=="3")]<-5#5 in w
BUSseq_col <- color_by_celltype_ramp[c(1,4,7,10,13)]

table(w_true,w_MNN)
w_MNN_shift<-w_MNN
w_MNN_shift[which(w_MNN=="2")]<-1#1 in w
w_MNN_shift[which(w_MNN=="5")]<-2#2 in w
w_MNN_shift[which(w_MNN=="4")]<-3
w_MNN_shift[which(w_MNN=="1")]<-4#3 in w
w_MNN_shift[which(w_MNN=="7")]<-5
w_MNN_shift[which(w_MNN=="3")]<-6#4 in w
w_MNN_shift[which(w_MNN=="6")]<-7#5 in w
MNN_col <- color_by_celltype_ramp[c(1,4,6,7,8,10,13)]

table(w_true,w_Seurat)
w_Seurat_shift<-w_Seurat
w_Seurat_shift[which(w_Seurat=="0")]<-1#1 in w
w_Seurat_shift[which(w_Seurat=="3")]<-2
w_Seurat_shift[which(w_Seurat=="7")]<-3
w_Seurat_shift[which(w_Seurat=="4")]<-4#2 in w
w_Seurat_shift[which(w_Seurat=="10")]<-5
w_Seurat_shift[which(w_Seurat=="5")]<-6
w_Seurat_shift[which(w_Seurat=="2")]<-7#3 in w
w_Seurat_shift[which(w_Seurat=="12")]<-8
w_Seurat_shift[which(w_Seurat=="9")]<-9
w_Seurat_shift[which(w_Seurat=="1")]<-10#4 in w
w_Seurat_shift[which(w_Seurat=="8")]<-11
w_Seurat_shift[which(w_Seurat=="11")]<-12
w_Seurat_shift[which(w_Seurat=="6")]<-13#5 in w
Seurat_col <- color_by_celltype_ramp[c(1:13)]

table(w_true,w_ZINBW)
w_ZINBW_shift<-w_ZINBW
w_ZINBW_shift[which(w_ZINBW=="4")]<-1#1 in w
w_ZINBW_shift[which(w_ZINBW=="5")]<-2
w_ZINBW_shift[which(w_ZINBW=="2")]<-3#2 in w_true
w_ZINBW_shift[which(w_ZINBW=="-1")]<-4#3 in w_true
w_ZINBW_shift[which(w_ZINBW=="3")]<-5#4 in w_true
w_ZINBW_shift[which(w_ZINBW=="1")]<-6#5 in w_true
ZINBW_col <- color_by_celltype_ramp[c(1,2,4,7,10,13)]
