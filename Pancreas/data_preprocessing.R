...
# donor_id<-table(marker_uber$Donor)
# plot(gse81076.uber$Dim1,gse81076.uber$Dim2,pch=19,col=palette()[rep(2:9,donor_id)])
# 
# celltype_id <- rep(NA,nrow(gse81076.uber))
# celltype_id[which(marker_uber$CellType=="")]<-1
# celltype_id[which(marker_uber$CellType=="Alpha")]<-2
# celltype_id[which(marker_uber$CellType=="Beta")]<-3
# celltype_id[which(marker_uber$CellType=="Delta")]<-4
# celltype_id[which(marker_uber$CellType=="Acinar")]<-5
# celltype_id[which(marker_uber$CellType=="Ductal")]<-6
# celltype_id[which(marker_uber$CellType=="Mesenchyme")]<-7
# celltype_id[which(marker_uber$CellType=="PP")]<-8
# plot(gse81076.uber$Dim1,gse81076.uber$Dim2,pch=19,col=celltype_id)
...
donor_id<-table(marker_uber$Donor)
donor_col <- palette()[rep(2:5,donor_id)]
plot(marker_uber$Dim1,marker_uber$Dim2,pch=19,col=palette()[rep(2:5,donor_id)])

celltype_id <- rep(NA,nrow(marker_uber))
celltype_id[which(marker_uber$CellType=="")]<-1
celltype_id[which(marker_uber$CellType=="Alpha")]<-2
celltype_id[which(marker_uber$CellType=="Beta")]<-3
celltype_id[which(marker_uber$CellType=="Delta")]<-4
...
celltype_id[which(marker_uber$CellType=="Mesenchyme")]<-7
celltype_id[which(marker_uber$CellType=="PP")]<-8
plot(marker_uber$Dim1,marker_uber$Dim2,pch=19,col=celltype_id)
legend("bottomright",legend=c("Alpha","Beta","Delta","Acinar","Ductal","Mesenchyme","PP"),pch=19,col = 2:8)

plot(marker_uber$Dim1[which(marker_uber$CellType=="Beta")],marker_uber$Dim2[which(marker_uber$CellType=="Beta")],pch=19,col=celltype_id[which(marker_uber$CellType=="Beta")])

plot(marker_uber$Dim1[which(marker_uber$CellType=="Delta")],marker_uber$Dim2[which(marker_uber$CellType=="Delta")],pch=19,col=donor_col[which(marker_uber$CellType=="Beta")])

plot(marker_uber$Dim1[1:352],marker_uber$Dim2[1:352],pch=19,col=celltype_id[1:352])


plate_list<-strsplit(marker_uber$Plate,"_")
plate_id<-NULL
for(i in 1:length(plate_list)){
  plate_id <- c(plate_id,plate_list[[i]][1])
}
lib_id<-paste0(marker_uber$Donor,"_",plate_id)

# cell_match<-match(rownames(read),marker_uber$Sample)
# subtype_true<-marker_uber$CellType[cell_match]
...
save(all.met, file = "./RawCountData/pancreas_metadata.RData")
